...
- **入力パラメータ**:
  - `repoPath`: Gitリポジトリのパス（必須）
  - `limit`: 取得するコミット数（オプション、デフォルト: 20）
  - `branch`: 対象ブランチ名（オプション、デフォルト: 現在のブランチ）
  - `author`: 著者名でフィルタ（オプション）
  - `since`: 指定日時以降のコミット（オプション、ISO 8601形式）
  - `until`: 指定日時以前のコミット（オプション、ISO 8601形式）
  - `grep`: コミットメッセージの検索文字列（オプション）
  - `oneline`: 簡略表示フラグ（オプション、デフォルト: false）
- **出力**: コミット履歴
  - `commits`: コミット情報の配列
    - `hash`: コミットハッシュ
    - `author`: 著者名
    ...
    - `message`: コミットメッセージ
    - `parentHashes`: 親コミットのハッシュ配列
    - `branch`: ブランチ名（onelineがfalseの場合）
    - `stats`: 変更統計（onelineがfalseの場合）
      - `files`: 変更されたファイル数
      - `insertions`: 追加行数
      - `deletions`: 削除行数
  - `total`: 取得したコミット数
