...
describe('mergeBranch', () => {
    describe('successful merge scenarios', () => {
        ...
        it('should perform a merge with custom target branch', async () => {
            ...
            vi.mocked(simpleGit).mockReturnValue(mockGit);
            const result = await mergeBranch('/repo/path', 'source-branch', 'target-branch');
            expect(result.isOk()).toBe(true);
            if (result.isOk()) {
        ...
        it('should perform a fast-forward merge when strategy is specified', async () => {
            ...
            vi.mocked(simpleGit).mockReturnValue(mockGit);
            const result = await mergeBranch('/repo/path', 'source-branch', undefined, 'fast-forward');
            expect(result.isOk()).toBe(true);
            if (result.isOk()) {
        ...
        it('should perform a squash merge when strategy is specified', async () => {
            ...
            vi.mocked(simpleGit).mockReturnValue(mockGit);
            const result = await mergeBranch('/repo/path', 'source-branch', undefined, 'squash');
            expect(result.isOk()).toBe(true);
            if (result.isOk()) {
        ...
        it('should use custom merge message when provided', async () => {
            ...
            const result = await mergeBranch('/repo/path', 'source-branch', undefined, 'merge', customMessage);
            expect(result.isOk()).toBe(true);
        ...
        it('should perform merge without commit when noCommit is true', async () => {
            ...
            vi.mocked(simpleGit).mockReturnValue(mockGit);
            const result = await mergeBranch('/repo/path', 'source-branch', undefined, 'merge', undefined, true);
            expect(result.isOk()).toBe(true);
            if (result.isOk()) {
    ...
    describe('error scenarios', () => {
        it('should return error when source branch does not exist', async () => {
            ...
            if (result.isErr()) {
                expect(result.error.message).toContain('Source branch does not exist: non-existent-branch');
            }
        });
        it('should return error when target branch does not exist', async () => {
            ...
            vi.mocked(simpleGit).mockReturnValue(mockGit);
            const result = await mergeBranch('/repo/path', 'source-branch', 'non-existent-target');
            expect(result.isErr()).toBe(true);
            if (result.isErr()) {
                expect(result.error.message).toContain('Target branch does not exist: non-existent-target');
            }
        });
