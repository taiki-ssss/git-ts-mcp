...
describe('createGitBranchMergeHandler', () => {
    describe('successful handler operations', () => {
        it('should merge branches successfully with minimal parameters', async () => {
            ...
            expect(lib.mergeBranch).toHaveBeenCalledWith('/test/repo', 'feature-branch', undefined, 'merge', undefined, false);
        ...
        it('should merge branches with all parameters specified', async () => {
            ...
            expect(lib.mergeBranch).toHaveBeenCalledWith('/test/repo', 'feature-branch', 'develop', 'fast-forward', 'Custom merge message', false);
            expect(result).toEqual({
                content: [
                    {
                        type: 'text',
        ...
        it('should handle merge without commit', async () => {
            ...
            expect(result.content[0].text).toContain('Successfully merged feature-branch into main');
    ...
    describe('error handling', () => {
        ...
        it('should handle merge conflicts', async () => {
            vi.mocked(lib.mergeBranch).mockResolvedValue(err(new Error('Merge conflict occurred. Conflicted files: file1.txt, file2.txt')));
            const handler = createGitBranchMergeHandler();
            const result = await handler({
                repoPath: '/test/repo',
                sourceBranch: 'feature-branch',
            });
            expect(result.content[0].text).toBe('Merge conflict occurred. Conflicted files: file1.txt, file2.txt');
        });
        it('should handle other merge errors', async () => {
            vi.mocked(lib.mergeBranch).mockResolvedValue(err(new Error('Source branch does not exist')));
            const handler = createGitBranchMergeHandler();
            const result = await handler({
                repoPath: '/test/repo',
