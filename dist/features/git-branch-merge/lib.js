...
export async function mergeBranch(repoPath, sourceBranch, targetBranch, strategy = 'merge', message, noCommit = false) {
    try {
        ...
        try {
            ...
            if (errorMessage.includes('CONFLICTS') || errorMessage.includes('conflict')) {
                ...
                return err(new Error(`Merge conflict occurred. Conflicted files: ${status.conflicted.join(', ')}`));
            }
            // Other merge errors
