...
export function createGitServer() {
    ...
    server.tool('git_commit', 'Create a git commit in the specified repository', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
        message: z.string().min(1).describe('Commit message'),
    }, gitCommitHandler);
    // Register git_status tool
    server.tool('git_status', 'Get the status of a git repository', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
    }, gitStatusHandler);
    // Register git_add tool
    server.tool('git_add', 'Add files to the git staging area', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
    ...
    server.tool('git_branch_merge', 'Merge git branches', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
    ...
    server.tool('git_log', 'Get commit history from a git repository', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
    ...
    server.tool('git_checkout', 'Switch branches or restore working tree files', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
    ...
    server.tool('git_push', 'Push changes to remote repository', {
        repoPath: z.string().min(1).describe('Path to the git repository'),
